	...
	<form action="{{ 'edit/store' }}" method="POST" enctype="multipart/form-data">
		...
		<div class="edit_user row">
			...
			@foreach($users as $user)
				...
				<input id="email" class="form-control  mb-5" value="{{ $user->email }}">
				<small>
					<p class="font-weight-light">パスワードを変更する場合は入力してください。</p>
				</small>
				<label for="password">{{ __('現在のパスワード') }}</label>
				<input id="password" class="form-control  mb-2" value="">
				<label for="new_password">{{ __('新しいパスワード') }}</label>
				<input id="new_password" class="form-control  mb-5">
		...
		<span class="border-right"></span>
			<div class= "edit_user is_address col col-md-3 ">
				...
				<input id="name" class="form-control  mb-2" value="{{ $user->name }}">
				<label for="furigana">{{ __('ふりがな') }}</label>
				<input id="furigana" class="form-control  mb-2" value="{{ $user->furigana }}">
				<label for="tel">{{ __('電話番号') }}</label>
				<input id="tel" class="form-control  mb-2" value="{{ $user->tel }}">
				<label for="post_code">{{ __('郵便番号') }}</label>
				<input id="post_code" class="form-control  mb-2" value="{{ $user->post_code }}">
