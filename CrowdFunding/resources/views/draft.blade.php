						...
						<div class="contact_form row">
							<div class= "contact_form is_products w-75 mx-auto">
								<div class="form-group form-content">
									...
									<textarea class="form-control rounded-0 mb-4" id="return" rows="3" placeholder="" value="{{ old('return') }}"></textarea>
									...
									<textarea class="form-control rounded-0 mb-2" id="faq2" rows="3" placeholder="クラウドファンディング経験のある方からの紹介の場合もお書きください(審査が通りやすくなります)" value="{{ old('faq2') }}"></textarea>
		...
		<div class="tab-pane fade" id="pills-not-products" role="tabpanel" aria-labelledby="pills-not-products-tab">
			...
			<div class= "container">
				...
				<form action="{{ 'store' }}" method="POST">
					...
					<form>
						<div class="contact_form row">
							<div class= "contact_form is_no_products w-75 mx-auto">
								<div class="form-group form-content">
									<label for="name" class="font-weight-bold">{{ __('氏名') }}</label>
									<span class="required-item text-danger">*</span>
									<input id="name" class="form-control rounded-0 mb-4" placeholder="例：山田　太郎" value="{{ old('name') }}">
									...
									<span class="required-item text-danger">*</span>
									<textarea class="form-control rounded-0 mb-3" id="profile" rows="5" placeholder="" value="{{ old('profile') }}"></textarea>
									<label for="job" class="font-weight-bold">{{ __('ご職業') }}</label>
									<span class="required-item text-danger">*</span>
									<textarea class="form-control rounded-0 mt-2 mb-3" id="job" rows="2" placeholder="" value="{{ old('job') }}"></textarea>
									<label for="twitter" class="font-weight-bold">{{ __('Twitter') }}</label>
									<textarea class="form-control rounded-0 mb-3" id="twitter" rows="2" placeholder="アカウントを複数お持ちの場合はすべて入力してください。" value="{{ old('twitter') }}"></textarea>
									<label for="facebook" class="font-weight-bold">{{ __('Facebook') }}</label>
									<textarea class="form-control rounded-0 mb-3" id="facebook" rows="2" placeholder="アカウントを複数お持ちの場合はすべて入力してください。" value="{{ old('facebook') }}"></textarea>
									<label for="instagram" class="font-weight-bold">{{ __('Instagram') }}</label>
									<textarea class="form-control rounded-0 mb-3" id="instagram" rows="2" placeholder="アカウントを複数お持ちの場合はすべて入力してください。" value="{{ old('instagram') }}"></textarea>
									<label for="web_page" class="font-weight-bold">{{ __('その他WEBサイト') }}</label>
									<textarea class="form-control rounded-0 mb-5" id="web_page" rows="2" placeholder="複数お持ちの場合はすべて入力してください。" value="{{ old('web_page') }}"></textarea>
									...
									<textarea class="form-control rounded-0 mb-5" id="overview" rows="5" placeholder="実施したい企画、タイトル案、リターン案など" value="{{ old('overview') }}"></textarea>
